...
const validateComprador = () => {
    const name = document.getElementById("comprador").value;
    // console.log("comprador: ", name);
    if (!(name.trim()) || (name.trim()).length > 80) {
        return [false, "El nombre del comprador no puede estar vacío y debe tener menos de 80 caracteres"];
    }
    return [true, ""];
}
...
const validateForm = () => {
    ...
    if (!isValid) {
        ...
        confirmationModal("El formulario es válido :)", "¿Confirma el registro de este pedido?", "Sí, confirmo.", "No, quiero volver al formulario.");
    }
}
...
// -- submit form --
// gets called by modal (same hack as before, sowwy uwu)
const submitForm = () => {
    ...
    fetch("/agregar-pedido", {
        ...
        if (response.ok) {
            finishModal("Hemos recibido el registro de pedido.", "Muchas gracias!");
        } else {
